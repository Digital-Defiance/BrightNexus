...
// BrightNexus
//
// Decoded plaintext shape of a v3 SDI payload (RFC §5).
//
// In v3 the entire payload — including the `type` and `context` schema
...
/// Top-level SDI payload as it appears in `EphemeralStore`.
struct BrightLinkPayload {
    ...
    static func decode(plaintext: Data, type: String, context: String) throws -> BrightLinkPayload {
        ...
        // the flat fields (legacy SDIAgent shape). Accept both.

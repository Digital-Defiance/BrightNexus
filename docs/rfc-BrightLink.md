...
The bridge keeps its EBP/1 command surface unchanged; SDI-EB adds new commands inside that envelope (`SDI_REGISTER`, `SDI_PUSH`, the geo-socket subset). A v3-aware client library MAY expose them through the same `EnclaveBridgeClient` class that already speaks EBP/1, or through a sibling class. v3 is greenfield — there is no legacy compatibility surface to preserve, so a bridge that returns `Unknown command` for v3 SDI commands is simply an out-of-date EBP/1-only bridge.
...
1. **Local Channel.** The bridge hosts an `AF_UNIX`/`SOCK_STREAM` socket exclusively accessible by the local user (filesystem permissions enforced by macOS). The path is the canonical BrightNexus path:

   ```
   $HOME/.brightchain/brightnexus/brightnexus.sock
   ```

   Clients MAY override via the `BRIGHTNEXUS_SOCKET` environment variable. There are no legacy fallback paths in v3: the protocol is greenfield (no users to migrate from EBP/1-only Enclave Bridge installations) and the legacy `~/.enclave/enclave-bridge.sock` and `/tmp/enclave-bridge.sock` paths from earlier drafts are not honored. The bridge's socket is **not** namespaced per shell session; multiple concurrent SDI sessions multiplex on the same socket as separate EBP/1 connections, each holding its own per-connection state (§4.3).
...
The bridge MUST enforce the following rate limits, each tracked per the scoping unit indicated:

| Surface | Threshold | Scope | Action on breach |
|---|---|---|---|
| Failed `SDI_REGISTER` attempts | 10 per minute | per connecting PID | close connection, log warning |
| Failed `SDI_INGEST` verifications | 30 per minute | per session | tear down session, log warning, require re-register |
| Failed `SDI_PUSH` operations | 30 per minute | per session | tear down session, log warning, require re-register |

Successful `SDI_INGEST` and `SDI_PUSH` operations are NOT rate-limited at the protocol layer — legitimate workflows may burst (e.g. provisioning credentials for many services from a single CI script). Implementations MAY add their own rate limits at the application layer (e.g. UI throttling) but MUST NOT reject otherwise-valid requests below the failure thresholds above.

The "failed" counters above are reset on session tear-down; they do not persist across re-registration. The `SDI_REGISTER` failure counter is reset when the connection closes.
...
**Strict 33-byte-only.** SDI-EB v3 senders MUST emit only the 33-byte compressed canonical form for ECIES ephemeral keys, and BrightNexus decoders MUST reject any other shape with `Invalid ephemeral public key format`. The DD-ECIES §5.3 backward-compat language permits a *general* DD-ECIES decoder to accept 33-byte compressed (canonical), 65-byte uncompressed, and 64-byte raw ephemeral keys; BrightNexus does not exercise that tolerance for any of its envelope-decoding paths (neither the SDI-EB v3 `SDI_REGISTER` envelope nor the general-purpose `ENCLAVE_DECRYPT` command). The reasons:

1. v3 is greenfield. There are no v0/v1/v2 senders that emit 65-byte or 64-byte ephemeral keys to migrate from.
2. `node-ecies-lib` v4 — the canonical DD-ECIES sender — already emits 33-byte compressed only. Tolerating other forms would be insurance against a sender population that does not exist.
3. A stricter parser has fewer attack surfaces and produces clearer diagnostics. A malformed envelope fails immediately with `Invalid ephemeral public key format` rather than parsing into a misshapen state and failing later with a misleading `Decryption failed`.

A future revision MAY relax this if a population of legacy senders ever materializes; until then, strict.
...
### 4.6 OSC 7777 Wire Format and Capture Architecture

OSC 7777 is the v3 envelope for SDI payloads on every transport: `bsh-inject` direct delivery, subprocess stdout, and bridge → shell push. The format is byte-for-byte identical to v2 so that v2-aware tooling that already emits OSC 7777 sequences keeps working.
...
| `base64-counter` | standard Base64 | 8-byte big-endian unsigned monotonic sequence counter (per direction; see §4.6.4). |
...
`\a` (BEL, 0x07) is the canonical sequence terminator. Implementations MAY also accept `\e\\` (ST, the OSC standard terminator) for tolerance, but MUST emit BEL.

#### 4.6.1 Capture Architecture (v3 — Replaces v2 §3.3 Terminal-Emulator Assumption)

v2 assumed terminal emulators would forward OSC 7777 sequences to a daemon over a side channel. No mainstream emulator did. v3 instead designates **the shell itself** as the OSC 7777 capture point. Two ingestion paths flow into the same bridge `SDI_INGEST` endpoint, sharing one session and one outbound counter:

**Path A — `bsh-inject` direct delivery (fast path, the common case).**
The `bsh-inject` builtin runs in-process inside the shell. It has direct access to the shell's already-registered SDI session, builds the OSC 7777 sequence under `K_session`, increments the shared outbound counter, and sends `SDI_INGEST` to the bridge over the existing EBP/1 socket. No subprocess, no PTY round-trip. `bsh-inject` MAY in addition write the sequence to `/dev/tty` for terminal-resident consumers; the canonical v3 delivery is to the bridge.

**Path B — PTY-proxy stream filter (open extension point for third-party tools).**
The reference shell (`bsh`) acts as a PTY proxy: foreground commands run in a child PTY whose master the shell holds. Subprocess stdout passes through a stream filter inside the shell before reaching the user's terminal. The filter recognises the OSC 7777 introducer mid-stream, accumulates to BEL, **strips the sequence from the bytes written to the terminal** (so it never reaches scrollback, recordings, or `tee`-ed output), and forwards the captured raw bytes to the bridge as `SDI_INGEST`. Subprocesses that already know how to emit OSC 7777 require zero awareness of the bridge or session — they just write to stdout.

Both paths increment the same `c_shell_to_agent` counter. The bridge's replay window does not care which path delivered which sequence; it only requires strict monotonicity within the direction. This lets the two paths interleave cleanly without coordination beyond shared shell-session state.

The bridge cannot read the user's PTY directly (macOS does not give an arbitrary background process the FD of a foreground shell's controlling terminal). The shell-as-capture-point design closes that gap by routing every OSC 7777 sequence through the shell on its way to the bridge.

#### 4.6.2 Direction Determination

A sequence read by the bridge from `SDI_INGEST` is `Shell → Agent` (`dir_tag = 0x01`) regardless of which capture path delivered it. A sequence read by the shell from a `SDI_PUSH` push event (§4.7) is `Agent → Shell` (`dir_tag = 0x02`). The receiver always uses its own direction tag for AAD reconstruction during decryption — an attacker who replays a captured sequence in the wrong direction gets a GCM-tag mismatch.

> **Note on `type` field confidentiality.** The `type` field is transmitted in plaintext within the OSC 7777 wire bytes. On the `SDI_INGEST` socket path it appears in the JSON-wrapped `sequence` field of the request; on the `bsh-inject --emit-stdout` path it appears in the byte stream visible to anything reading bsh's stdout. Implementations that consider the payload schema identifier sensitive SHOULD use a generic `type` value (e.g. `sdi-payload`) and encode the true type inside the encrypted JSON body. The bridge honours body-side `type` and `context` overrides per §4.6 and §5.

#### 4.6.3 Additional Authenticated Data (AAD)
...
- `counter_bytes` is the raw 8-byte big-endian counter from the appropriate direction (§4.6.4).
...
#### 4.6.4 Replay Protection via Per-Direction Monotonic Counters

Each session maintains **two** independent monotonic counters, one per direction:

- `c_shell_to_agent` — incremented by the shell on every emit through either Path A or Path B (§4.6.1); validated by the bridge on receive.
...
1. Its own outbound counter for the direction it emits in (used to fill `counter_bytes` on emit, incremented by 1 per emit). When the shell uses Paths A and B simultaneously, both share the single `c_shell_to_agent` counter — the shell is responsible for serialising counter allocation across both paths so each emitted sequence carries a unique value.
...
- Reconstructs AAD using its receiving direction's `dir_tag` (§4.6.3).
...
The bridge cannot write to the PTY directly without help from the shell: macOS does not give an arbitrary background process the FD of a foreground shell's controlling terminal. v3 brings the relay explicitly into the EBP/1 surface as a **client-pulled push channel**.

The mechanism: the client opens a long-lived "push subscription" by calling `SDI_PUSH` with `op="subscribe"`. The bridge holds the connection open and writes one EBP/1 response per push event. Each event carries a fully-formed OSC 7777 sequence (encrypted under `K_session` with `dir_tag = 0x02`, Agent → Shell). The client (the shell, in the canonical v3 deployment) verifies the sequence under `K_session`, decodes the payload, and dispatches it locally — typically routing the plaintext to the shell's own SDI handler for `geo-context` zone transitions (§6) or other agent-originated events.

The client is NOT required to write the received sequence to `/dev/tty`. v2 documented this as the canonical relay path on the assumption that other PTY readers also wanted the payload. In v3 the shell IS the SDI dispatcher, so the in-process decode + dispatch path is sufficient. A client MAY still write the sequence to `/dev/tty` for diagnostic / passthrough purposes; doing so does not change the AAD or counter bookkeeping (the bridge has already incremented its outbound counter when emitting the push).
...
`sequence` is the complete OSC 7777 wire string with proper escapes (`\u001b` for `\e`, `\u0007` for `\a`). The client decodes the string back to bytes, parses it under `K_session`, and dispatches the plaintext payload locally.
...
### 4.8 Injection Interface

The reference shell (`bsh`) exposes a builtin `bsh-inject` that implements the full encrypt-and-deliver pipeline for Path A (§4.6.1):

```
bsh-inject --type <type> --context <url>
```

The JSON payload is read from **stdin**. The builtin performs lazy session initialisation (§4.5 if not already registered with the bridge), allocates the next `c_shell_to_agent` counter value (serialising against the PTY-proxy stream filter on Path B), encrypts with `K_session`, builds the OSC 7777 sequence, and sends `SDI_INGEST` to the bridge.

By default, `bsh-inject` does NOT additionally write the sequence to stdout. The bridge has the payload; no terminal observer needs it. If terminal-resident emission is wanted (debugging, custom downstream consumers in a multiplexer, deliberate visibility-through-the-terminal, test capture), pass `--emit-stdout` to additionally write the OSC 7777 bytes to file descriptor 1 (stdout). The flag does not change the bridge delivery — both happen, and both share the same `c_shell_to_agent` counter value.
...
**Bridge-restart resilience.** Because the bridge restart destroys all `K_session` instances, `bsh-inject` MUST detect a stale-session error (`SDI_INGEST` returning `AES-GCM authentication failed` after a long gap, or `ECONNREFUSED` on the bridge socket) and re-register transparently before retrying. The retry MUST NOT loop indefinitely; after one re-registration attempt, persistent failure MUST exit non-zero with a clear stderr message.

### 4.9 The `SDI_INGEST` EBP/1 Command (Shell → Agent Channel)

The shell delivers OSC 7777 sequences captured on either Path A (`bsh-inject` builtin) or Path B (PTY-proxy stream filter) to the bridge as one-shot `SDI_INGEST` requests. This is the canonical Shell → Agent transport in v3.

`SDI_INGEST` is a strict additive command — it does not replace `bsh-inject`'s direct-to-PTY emission, which remains the canonical delivery path for terminal-resident OSC 7777 dispatch. Clients SHOULD use `SDI_INGEST` only when the bridge itself is the intended consumer.
...
`sequence` is the complete OSC 7777 wire string (introducer through BEL terminator) constructed under the connection's bound `K_session` with `dir_tag = 0x01` (Shell → Agent). The bridge MUST:

1. Verify a session is bound to this connection (return `Session not registered on this connection` otherwise).
2. Verify the session has not expired (return `Session expired` otherwise).
3. Parse the OSC 7777 sequence per §4.6 with `expectedDirTag = 0x01` and `lastAcceptedCounter = c_shell_to_agent` for the session.
4. Reject malformed sequences, AAD authentication failures, replayed counters, and counters beyond the §4.6.4 replay window using the precise error strings tabulated in §4.9.3.
5. On success, advance `c_shell_to_agent` to the accepted counter, decode the JSON body, and route the payload to the bridge's local payload handler (in BrightNexus, `EphemeralStore`).
...
The bridge echoes back the resolved `type` and `context`. Per §4.6 nota bene, if the encrypted JSON body re-declares its own `type` or `context` fields, those values win over the OSC plaintext fields. The response MUST reflect the resolved (post-override) values so the client can confirm what the bridge actually stored.

#### 4.9.3 Errors

| Error | Condition |
| --- | --- |
| `{"error":"Missing sequence"}` | The request omitted the `sequence` field. |
| `{"error":"Session not registered on this connection"}` | No session bound; the connection has not run `SDI_REGISTER`. |
| `{"error":"Session expired"}` | Bridge clock is past `expiresAtUnix`. |
| `{"error":"Malformed OSC 7777 sequence: <reason>"}` | Sequence failed structural parse (missing introducer, wrong field count, non-base64 fields, etc.). |
| `{"error":"sessionId mismatch"}` | The 32-hex sessionId in the sequence does not match the session bound to this connection. |
| `{"error":"AES-GCM authentication failed"}` | The AAD/tag verification failed. Same hard fail as `Decryption failed` for ENCLAVE_DECRYPT — does not distinguish between wrong key, wrong direction, wrong counter binding, etc., to avoid leaking which axis is wrong. |
| `{"error":"Counter replayed"}` | The packet's counter is `≤ lastAcceptedCounter`. |
| `{"error":"Counter out of replay window"}` | The packet's counter is `> lastAcceptedCounter + 1000`. |
| `{"error":"Invalid payload body: <reason>"}` | The decrypted body failed JSON parse. |
| `{"error":"Peer attestation failed"}` | Enforce-mode peer attestation rejected the connection (§4.9.5). Never returned in default log-only mode. |

#### 4.9.4 Relationship to the Capture Paths

`SDI_INGEST` is the wire delivery of OSC 7777 sequences captured on either Path A or Path B (§4.6.1). The bridge does not distinguish between the two — both arrive as the same JSON request with the same `sequence` payload, both increment the same `c_shell_to_agent` counter, both validate against the same replay window. The shell is responsible for serialising counter allocation across the two paths; the bridge enforces strict monotonicity and rejects counter collisions as replays.

When `bsh-inject` is invoked with `--emit-stdout` (§4.8), the same sequence is additionally written to stdout (fd 1). The stdout copy and the bridge-bound copy carry the same counter value; the bridge sees only one ingest per emit.

#### 4.9.5 Peer Attestation, TTL Clamping, and Provenance

The bridge MUST attest the connecting peer on every `SDI_INGEST` and record the result in the audit log. On macOS this means: read the connecting PID via `getsockopt(LOCAL_PEERPID)`, resolve the executable path via `proc_pidpath()`, and read the binary's code-signing identity via `SecCodeCopyPath` / `SecStaticCodeCheckValidity`. On Linux the equivalent is `SO_PEERCRED` plus reading `/proc/<pid>/exe`.

Default policy is **log-only**: the bridge records `{pid, executablePath, codeSigningIdentity, signatureValid}` in the audit log alongside the ingested credential, surfaces the executable path in the menu-bar entry as a provenance hint, but does NOT reject ingests from unsigned binaries. This avoids breaking interactive use of unsigned tooling (e.g. `bsh-inject` during development, integration test harnesses, vendor-built scripts the user trusts).

Implementations MUST expose a setting that flips the default to **enforce mode**, where ingests from binaries that fail signature validation, fall outside an allowlist, or cannot be attested at all return `{"error":"Peer attestation failed"}`. The setting is opt-in; users who care about a hardened policy enable it explicitly.

Independently of policy mode, the bridge MUST clamp the per-credential `ttl` field at decode time to a configurable ceiling, default **3600 seconds (1 hour)**. A payload body declaring `"ttl": 86400` results in a stored entry whose `expiresAt` is `bridgeIssuedAt + 3600`. The clamp is applied silently — the response still echoes the resolved type/context, and the audit log records both the requested and clamped values. Rationale: individual credentials should be short-lived even when the registering session is not (the session TTL ceiling is 8 hours per §4.1).

Bridges that surface ingested credentials in a UI MUST display the resolved `expiresAt` (post-clamp), not the requested TTL. Users SHOULD see the actual lifetime of the entry, not the body's claim about it.

#### 4.9.6 Memory Hygiene

`K_session` is sensitive material. Bridges MUST `mlock` the bytes (or platform equivalent) for the session's lifetime to prevent the OS from paging them to disk, and MUST overwrite them with zeros via `memset_s` or equivalent compiler-fence-protected zeroization before the session record is freed. Replacing a `SymmetricKey` reference with a zero-filled key is insufficient — the original byte storage may persist in heap memory until the runtime garbage-collects it.

Implementations on platforms that don't expose `mlock` (or where `mlock` fails for the calling user's privilege) SHOULD log a warning at startup but MAY proceed; the absence of memory locking does not invalidate the cryptographic guarantees, only the post-compromise forensic resistance.
...
This RFC defines nine payload types: `ephemeral-auth` (§5.1), `db-connection` (§5.2), `geo-context` (§5.3), `api-token` (§5.4), `cloud-session` (§5.5), `ssh-credential` (§5.6), `kubeconfig-context` (§5.7), `totp-seed` (§5.8), and `mtls-cert` (§5.9). The first three are unchanged from v1/v2; §5.4 through §5.9 are new in v3 and reflect common workflows that previously fell outside the SDI surface (cloud SSO sessions, Kubernetes contexts, mTLS pairs, and so on).

**Type-identifier namespacing.** Type identifiers in this RFC use single-word or hyphenated lowercase tokens (`ephemeral-auth`, `kubeconfig-context`). These names are reserved for this RFC and any future revision. Implementations that define additional payload types MUST use a vendor-prefixed identifier of the form `<vendor>-<purpose>` where `<vendor>` is at minimum two characters and the prefix has at least one non-rfc match (e.g. `acme-internal-token`, `corpco-vault-grant`). Single-token identifiers without a hyphen are reserved; consumers MUST ignore unknown types and SHOULD log them at debug level.
...
Bearer-style API tokens for HTTP services. The most common credential form for modern dev workflows after `ephemeral-auth`.
...
```json
{
  ...
  "issued_at": 1748000000,
  "data": {
    "token": "ghp_x9kP4mQ2RtY8nL3vB7wZ...",
...
|---|---|---|---|
| `token` | string | yes | The raw token value. |
| `header_name` | string | no, default `"Authorization"` | HTTP header to inject the token into. |
| `prefix` | string | no, default `"Bearer "` | Verbatim prefix prepended to `token` when forming the header value. Set to `""` for raw tokens (e.g. some legacy APIs). |
| `scopes` | string[] | no | Free-form scope list, displayed in the menu bar but not enforced. |

Bridges SHOULD render the token as `••••••••` in the dropdown and reveal on click-to-copy. Click-to-copy MUST copy the bare `token`, not `<prefix><token>` — callers are responsible for header construction in their tooling.

### 5.5 `cloud-session`

Short-lived cloud-provider session credentials (AWS STS, GCP service-account access tokens, Azure AD tokens, etc.). Distinguished from `api-token` by the multi-field shape that mirrors what `aws sts assume-role` and equivalents return.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `provider` | string | yes | One of `"aws"`, `"gcp"`, `"azure"`, or a vendor-prefixed identifier per §5 namespacing. |
| `access_key_id` | string | yes (AWS), no (others) | AWS access key id. Other providers omit. |
| `secret_access_key` | string | yes (AWS), no (others) | AWS secret access key. Other providers omit. |
| `session_token` | string | yes | The session/access token bytes. For AWS this is the STS session token; for GCP the access token; for Azure the bearer token. |
| `region` | string | no | Cloud region/zone hint. |
| `expires_at` | integer | yes | Unix seconds. The bridge MUST clamp `ttl` such that the entry expires no later than `expires_at`. |

Bridges MUST render `secret_access_key` and `session_token` as masked. Click-to-copy returns the bare value.

### 5.6 `ssh-credential`

A complete SSH credential for one connection. Targeted at injecting deploy keys for short-lived sessions without writing the key to `~/.ssh/`.

```json
{
  "type": "ssh-credential",
  "context": "ssh://deploy@bastion.example.com:22",
  "ttl": 1800,
  "issued_at": 1748000000,
  "data": {
    ...
    "passphrase": "optional",
    "known_hosts_line": "bastion.example.com,203.0.113.42 ssh-ed25519 AAAA..."
  }
}
```
...
| `private_key_pem` | string | yes | OpenSSH-format private key (PEM). |
| `passphrase` | string | no | Passphrase for the private key. Empty/absent means unencrypted key. |
| `known_hosts_line` | string | no | Pre-pinned `known_hosts` line, to skip the TOFU prompt on first connect. |

Bridges MUST render `private_key_pem` and `passphrase` as masked. Click-to-copy on `private_key_pem` returns the full PEM block including the header/footer lines.

### 5.7 `kubeconfig-context`

A single Kubernetes context — cluster + user + namespace — suitable for transient `kubectl` access without merging into `~/.kube/config`.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `server` | string | yes | Cluster API server URL. |
| `cluster_name` | string | yes | Display name for the cluster. |
| `namespace` | string | no, default `"default"` | Default namespace for this context. |
| `user` | string | yes | Display name for the user identity. |
| `token` | string | one-of | Bearer token. Mutually exclusive with the cert/key pair. |
| `client_cert_pem` | string | one-of | mTLS client cert (PEM). |
| `client_key_pem` | string | one-of | mTLS client key (PEM). |
| `ca_pem` | string | yes | Cluster CA cert (PEM) for TLS verification. |

Exactly one of (`token`) or (`client_cert_pem` + `client_key_pem`) MUST be present. Bridges MUST render `token`, `client_key_pem`, and the body of `client_cert_pem` (but not the PEM header/footer) as masked.

### 5.8 `totp-seed`

A TOTP secret. The bridge SHOULD compute the current code on click-to-copy rather than copying the raw seed — this gives the user the workflow they actually want (the 6 digits, ready to paste) while keeping the seed itself off the clipboard.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `account` | string | yes | Account identifier shown in the menu-bar row. |
| `issuer` | string | yes | Issuer display name. |
| `secret_base32` | string | yes | Base32-encoded TOTP shared secret (RFC 6238 §3). |
| `period` | integer | no, default `30` | Time step in seconds. |
| `digits` | integer | no, default `6` | Number of digits in the generated code. |
| `algorithm` | string | no, default `"SHA1"` | One of `"SHA1"`, `"SHA256"`, `"SHA512"`. |

Bridges MUST render `secret_base32` as masked. Click-to-copy on the entry MUST copy the *current TOTP code* (not the seed). Bridges SHOULD additionally show the seconds-remaining countdown so the user knows when the code rolls.

### 5.9 `mtls-cert`

A client certificate + private key pair for mutual-TLS authentication to a specific endpoint.

```json
{
  "type": "mtls-cert",
  "context": "https://internal-api.example.com",
  "ttl": 3600,
  "issued_at": 1748000000,
  "data": {
...
| Field | Type | Required | Description |
|---|---|---|---|
| `endpoint` | string | yes | The HTTPS endpoint this credential is scoped to. |
| `cert_pem` | string | yes | Client certificate (PEM). |
| `key_pem` | string | yes | Client private key (PEM). |
| `key_passphrase` | string | no | Passphrase for `key_pem`. Empty/absent means unencrypted key. |
| `ca_bundle_pem` | string | no | Optional CA bundle for verifying the server cert chain. |

Bridges MUST render `key_pem` body and `key_passphrase` as masked. Click-to-copy on `cert_pem` and `ca_bundle_pem` MUST return the full PEM block including header/footer.

### 5.10 `plaintext`

Single-field generic carrier for short text fragments that don't fit a structured schema: backup codes, magic-link URLs, one-time usernames, signed URLs, password-reset tokens, vendor-issued installation keys, and so on.
...
| Field | Type | Required | Description |
|---|---|---|---|
| `label` | string | yes | Human-readable name shown in the menu-bar row. The OSC `context` field is for routing/identity (a URL, an account name); `label` is what the user reads when deciding what to copy. Max 64 chars. |
| `value` | string | yes | The string the user wants to copy. Max 4096 chars. |
| `masked` | boolean | no, default `false` | If `true`, the bridge displays `••••••••` in the menu bar; click-to-copy still returns the unmasked `value`. Senders MUST set this to `true` for OTP-style or recovery-code values; SHOULD set it for anything they consider secret-on-paper. |
| `kind` | string | no, default `"text"` | Soft hint for icon/category. Suggested values: `"text"`, `"code"`, `"url"`, `"token"`, `"key"`, `"note"`. Bridges that don't recognise the hint MUST treat it as `"text"`. |

Bridges MUST render exactly one menu-bar row per `plaintext` payload, with `label` as the row title and `value` (or `••••••••` if `masked`) as the secondary text. Click-to-copy on the row copies the unmasked `value`. Only one copy action is exposed (no submenu): if the sender wants multiple labelled values, they MUST send multiple `plaintext` payloads with distinct `context` values.

This type is intentionally minimal. Senders that find themselves wanting `data.fields[].{label, value}` should send multiple `plaintext` payloads with shared `context` prefixes. Bridges MAY group entries with a shared `context` prefix in the UI, but this is a presentation hint, not a schema rule.
...
| --- | --- | --- | --- |
| **Path file** | `~/.brightchain/brightnexus/brightnexus.geo.path` | `0600`, owner UID | A single line: the absolute path of the live socket. Stable name across restarts. |
| **Socket** | `~/.brightchain/brightnexus/brightnexus-<random>.geo.sock` | `0600`, owner UID | The actual Unix-domain socket. Random component preserves the squat-resistance property of §4.1. |

Bridge startup:

1. Generate random component, choose socket path under `~/.brightchain/brightnexus/`.
...
- **Tampered terminal stream.** A captured/forged OSC 7777 sequence in stdout (e.g. from `cat` of a hostile file). Defended by the AES-256-GCM authentication tag (§4.6) plus the bidirectional counter and AAD direction tag (§4.6.3 and §4.6.4).
...
- **Replay of captured `geo-context` sequences.** Defended by the per-direction monotonic counters (§4.6.4) and the BrightDate `issued_at`/`expires_at` window.
...
- **Replay Defense.** The per-direction monotonic counters (§4.6.4), the AAD-bound `dir_tag` (§4.6.3), and the `issued_at` timestamp (§5) together ensure that a captured OSC 7777 sequence cannot be re-submitted to its receiver, cannot be cross-replayed in the opposite direction, and cannot be resurrected after expiry. Counter replay attempts are logged as security events.

- **AAD Boundary Confusion Defense.** Length-prefixed AAD encoding (§4.6.3) including the leading direction tag prevents any two distinct `(dir, counter, type, context)` tuples from producing identical AAD bytes, closing the boundary confusion attack surface present in naive concatenation.
...
4. Maintain two per-direction monotonic counters (§4.6.4).
5. Construct AAD with the length-prefixed direction tag (§4.6.3). Note this is the SDI **OSC 7777** AAD; the **outer ECIES envelope** AAD is different and is defined by DD-ECIES via §4.5.0.
...
11. **Senders AND decoders MUST handle 33-byte compressed ephemeral keys only** in `SDI_REGISTER` envelopes. The DD-ECIES §5.3 backward-compat formats (65-byte uncompressed, 64-byte raw) are NOT honored inside SDI-EB v3 — see §4.5.0. Decoders MUST reject envelopes whose ephemeral key field is not exactly 33 bytes prefixed with `0x02` or `0x03` with the standard `Invalid ephemeral public key format` error.
...
## 15. Compatibility posture (greenfield)

SDI-EB v3 is **greenfield**. There are no users to migrate from earlier drafts, the legacy "Enclave Bridge" macOS app and the standalone "SDI Agent" daemon are both deprecated, and no third-party clients have shipped against the v3 surface. The protocol is therefore not constrained by the usual obligations to preserve backward-compatible code paths, accept legacy wire shapes, or maintain alternate filesystem layouts.

**What this means in practice:**

- Implementations MUST NOT bind a legacy `~/.enclave/enclave-bridge.sock` or `/tmp/enclave-bridge.sock` compatibility socket. If a v0/v1/v2 client attempts to connect to one of those paths, the connection MUST fail (no socket to find).
- Clients MUST NOT include legacy paths in their discovery order. The discovery list is exactly: `${BRIGHTNEXUS_SOCKET}` env override → `$HOME/.brightchain/brightnexus/brightnexus.sock`. No further fallbacks.
- Bridges MUST reject ECIES envelopes whose ephemeral key field is not 33-byte compressed. This applies uniformly to both the SDI-EB v3 `SDI_REGISTER` envelope (§4.5.0) and the general-purpose `ENCLAVE_DECRYPT` command. The DD-ECIES §5.3 65/64-byte tolerance is not honored by BrightNexus on any decode path.
- Implementations MUST NOT migrate state files from `~/.enclave/`. Bridges that find such files at startup MAY log a one-line notice, but they MUST NOT rename, copy, or read them.
- The wire `service` field in `HEARTBEAT` and `METRICS` is preserved as `"enclave-bridge"` for one specific reason: third-party EBP/1 clients written from the published EBP/1 specification still pin on that string. Pinning on `"enclave-bridge"` for liveness is fine; pinning on it for v3-awareness is not. v3-aware clients SHOULD pin on the new `"app": "brightnexus"` and `"sdiProtocolVersion": 3` fields in `VERSION`/`INFO` instead.

**What this does NOT mean:**

- DD-ECIES the standard remains broadly tolerant (§5.3). Other DD-ECIES decoders — for example a `node-ecies-lib` consumer outside the BrightNexus process — MAY still implement the 33/64/65-byte tolerance for general-purpose ECIES traffic. BrightNexus is one specific deployment of DD-ECIES that opts out of the tolerance. The DD-ECIES standard itself is unchanged.
- Future revisions of SDI-EB (v4 and beyond) will need their own compatibility posture statements. v4-greenfield will be the same kind of statement, made at v4-time, against v3-already-shipping reality.

If at some future point a deployment needs to support a true v3 → v4 migration with users involved, that migration story is out of scope for this document and MUST be specified separately. For now: there is exactly one wire format, exactly one filesystem layout, and exactly one set of socket paths.
...
| Issue | Resolution |
| --- | --- |
| No replay protection | Monotonic counter added to OSC sequence and AAD (§4.6, §4.6.4) |
| AAD boundary confusion via naive concatenation | Length-prefixed AAD encoding required (§4.6.3) |
...
| Single per-session monotonic counter | Two independent per-direction counters: `c_shell_to_agent`, `c_agent_to_shell` (§4.6.4). | A single counter with two writers cannot soundly distinguish legitimate emits from replays in the opposite direction. |
| AAD covers `(counter, type, context)` | AAD covers `(dir_tag, counter, type, context)` with `dir_tag` length-prefixed (§4.6.3). | Cross-direction replay is impossible: GCM tag verification fails when the receiver reconstructs AAD with its direction tag. |
...
| Geo socket lives at v2-defined `/run/user/<uid>/sdi-agent...` | Geo socket lives under `~/.brightchain/brightnexus/brightnexus-<random>.geo.sock` alongside the EBP/1 socket (§7.2) | Co-locates all bridge state under one BrightChain-namespaced directory. v3 is greenfield — no legacy compat surface. |
...
| `enclave/EnclaveBridge/ECIESKeyManager.swift` | secp256k1 private key persistence (`~/.brightchain/brightnexus/ecies-privkey.bin`). |
